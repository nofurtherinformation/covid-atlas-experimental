...
const dataFn = (numeratorData, numeratorProperty, index, range, denominatorData, denominatorProperty, denominatorIndex, denominatorRange, scale)  => {
  if (denominatorProperty===null&&range===null){ // whole count or number -- no range, no normalization
    return (numeratorData[index]||numeratorData[numeratorProperty])*scale
    ...
    return (numeratorData[index]||numeratorData[numeratorProperty])/(denominatorData[denominatorIndex]||denominatorData[denominatorProperty])*scale
    ...
    return ((numeratorData[index]-numeratorData[index-range])/range)/(denominatorData[denominatorIndex]||denominatorData[denominatorProperty])*scale
